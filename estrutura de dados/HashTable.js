...
class TabelaHash {
  ...
  // Função de hash DJB2: transforma chave em índice
  ...
  inserir(chave, valor) {
    ...
    console.log(`🔢 [inserir] Índice calculado para chave '${chave}': ${indice}`);
    // Remove chave se já existe (para atualizar)
  ...
  buscar(chave) {
    const indice = this.funcao_hash_hashDJB2(chave);
    console.log(`🔢 [buscar] Índice calculado para chave '${chave}': ${indice}`);
    ...
    while (atual) {
      if (typeof atual.valor === 'string' && atual.valor.startsWith(chave + ':')) {
        ...
        console.log(`✅ [buscar] Chave '${chave}' encontrada. Retornando valor '${valor}'.`);
        return valor;
      }
      atual = atual.proximo;
    }
    console.log(`❌ [buscar] Chave '${chave}' não encontrada. Retornando undefined.`);
    return undefined;
  }
  ...
  remover(chave) {
    const indice = this.funcao_hash_hashDJB2(chave);
    console.log(`🔢 [remover] Índice calculado para chave '${chave}': ${indice}`);
    this.tabela[indice].remover(chave);
  }
...
console.log('Buscar estado (não existe):', tabela.buscar('estado'));

console.log('\n--- Removendo elementos ---');
tabela.remover('idade');
console.log('Removido: idade');
tabela.remover('estado');
console.log('Tentativa de remover estado (não existe)');
